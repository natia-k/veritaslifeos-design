(() => {
  ...
  const productBackdrop = document.querySelector(".product-backdrop");
  ...
  const modeContent = {
    personal: {
      ...
      forumTags: ["Legal", "Career", "Family"],
      signalLabel: "Live signal",
      signalScore: "86",
      signalCopy: "confidence across active recommendations",
      priorityProgress: "78%",
      researchProgress: "86%",
      goalsProgress: "74%"
    },
    business: {
      ...
      forumTags: ["Pipeline", "Hiring", "Ops"],
      signalLabel: "Market signal",
      signalScore: "91",
      signalCopy: "confidence across active operating scans",
      priorityProgress: "82%",
      researchProgress: "91%",
      goalsProgress: "81%"
    }
  ...
  function applyModeContent(mode) {
    ...
    document.getElementById("signal-card-label").textContent = c.signalLabel;
    document.getElementById("signal-card-score").textContent = c.signalScore;
    document.getElementById("signal-card-copy").textContent = c.signalCopy;
    ...
    setTags("module-forum-tags", c.forumTags);
  }

  function setTheme(theme, save = true) {
    root.setAttribute("data-theme", theme);
    syncButtons("theme", theme);
    if (save) localStorage.setItem("veritas-theme", theme);
  }
  ...
  function setMode(mode, save = true) {
    ...
    if (save) localStorage.setItem("veritas-mode", mode);
  }
  ...
  if (window.innerWidth > 1180 && productStage && productWindow) {
    productStage.addEventListener("mousemove", (event) => {
      ...
      if (productBackdrop) {
        productBackdrop.style.transform =
          `perspective(1800px) rotateY(${rotateY - 1.5}deg) rotateX(${rotateX - 0.5}deg) scale(0.985) translateX(${translateX * 0.7}px) translateY(${translateY * 0.6}px)`;
      }
    });

    productStage.addEventListener("mouseleave", () => {
      productWindow.style.transform =
        "perspective(1800px) rotateY(-10deg) rotateX(5deg) translateX(0px) translateY(0px)";

      if (productBackdrop) {
        productBackdrop.style.transform =
          "perspective(1800px) rotateY(-10deg) rotateX(5deg) scale(0.985) translateX(0px) translateY(0px)";
      }
    });
  }
})();
